...
export default async function CoursesPage({ searchParams }: CoursesPageProps) {
  ...
  // Get enrollment counts and ratings
      ...
      <main className="flex-1">
        <div className="border-b border-border bg-muted/30 py-12">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <p className="mt-2 text-muted-foreground">Discover courses that will help you reach your goals</p>
          </div>
        </div>

        <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
          <Suspense fallback={<div>Loading filters...</div>}>
            <CoursesFilter universities={universities || []} categories={[]} />
          </Suspense>

          {/* Active filters */}
          {(params.search || params.university || params.level) && (
            <div className="mb-6 flex flex-wrap items-center gap-2">
              <span className="text-sm text-muted-foreground">Active filters:</span>
              {params.search && <Badge variant="secondary">Search: {params.search}</Badge>}
              {params.university && <Badge variant="secondary">University: {params.university}</Badge>}
              {params.level && <Badge variant="secondary">Level: {params.level}</Badge>}
            </div>
          )}

          {/* Results count */}
          <p className="mb-6 text-sm text-muted-foreground">Showing {coursesWithStats.length} courses</p>
